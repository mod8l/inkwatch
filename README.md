...
No printer? Either open the PNG full-screen on a tablet laid flat and play with a stylus, or **draw the board by hand** — just a 3×3 grid, about a hand-span square, in dark pen or marker. The agent finds the grid's own outer lines; nothing else is needed. Keep the whole grid in frame with a little margin, and if your lighting throws hard shadows, four solid black squares at the corners make detection more robust (see `NOTES.md`, hand-drawn fallback).
