...
**Demo video:** `recordings/inkwatch_scenarios.mp4` (generated by the scenario simulator below — every §8/§9 behavior in one composite video; unlisted YouTube/Drive link: TODO)
...
## Scenario simulator (a virtual human, no camera needed)

The strongest end-to-end check in the repo: a simulated human plays whole scripted scenarios against the **real, unmodified app**, which is launched with `--camera http://127.0.0.1:<port>/stream` (the documented MJPEG input used for phone cameras) and fed frames rendered from actual desk footage — real paper, real pencil, wobbly graphite strokes, sensor noise.

```bash
python tools/simrender.py --extract sessions/<id>   # once: pull the paper background + pencil sprite out of a --record'ed session
python tools/sim_scenarios.py                        # all runs: happy path, the §9 recovery gauntlet, live escalation, agent-first
python tools/simvideo.py                             # composite the footage into recordings/inkwatch_scenarios.mp4
```

The simulator listens to what the agent says (its printed speech lines, O4) and plays the human's side by hand: draws marks, lifts the pen mid-stroke, scribbles over taken cells, draws the agent's O in the wrong cell, lingers over the page, bumps it, erases committed ink, unplugs the camera for a few seconds. Every §8/§9 behavior is asserted as a PASS/FAIL verdict per scenario (75 checks in the recovery run alone); footage, logs, and verdicts land in `recordings/sim/<run>/`. `tests/test_simulate.py` pins the renderer↔perception contract in CI so the frames it feeds stay honest. Bugs it has caught and gotten fixed: an occupied-cell scribble that soft-locked the game after the warning, and a default escalation model id that no longer exists (every call fell back to asking).
...
tools/               printable sheet generator, calibration view, scenario simulator (simrender/simulate/sim_scenarios/simvideo)
