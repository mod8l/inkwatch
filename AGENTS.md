...
- CI (`.github/workflows/tests.yml`) runs on every push and every PR
  targeting `main`, as four separate checks — a PR shouldn't merge with
  any of them red:
  - `pytest (py3.11, latest compatible deps)` / `pytest (py3.12, ...)` —
    the full test suite with dependencies resolved fresh (no upper
    bounds in `pyproject.toml`), matrixed across both supported Python
    versions. This is the early-warning signal for a breaking upstream
    release.
  - `dependency lockfile (up to date, installs clean, tests pass)` —
    regenerates `requirements.txt`/`requirements-dev.txt` and fails if
    they drifted from `pyproject.toml`, then installs from the committed
    lock and runs the suite again against those exact pinned versions.
  - `dependency vulnerability scan (pip-audit)` — audits the locked
    dependency set (`requirements-dev.txt`), so results are reproducible
    rather than depending on whatever happens to resolve "latest" that
    day.
...
`requirements.txt` and `requirements-dev.txt` are pinned, reproducible
locks generated from `pyproject.toml` with [`pip-tools`](https://pip-tools.readthedocs.io/).
`pyproject.toml` stays the source of truth for *what* the project depends
on (unpinned, so `pip install -e .` — what `README.md`'s quick start
uses — always gets a stranger the latest compatible versions with no
extra tool to install); the lockfiles pin *exactly which* versions were
last verified to work together, for CI and for matching a reviewer's
environment to whoever opened the PR.

Regenerate both after changing `dependencies` or `optional-dependencies`
in `pyproject.toml`:
...
Commit the regenerated files in the same PR as the `pyproject.toml`
change — the `dependency lockfile` CI check fails otherwise. (The
autogenerated header comment in these files can say `--no-index` even
though the compile actually hit PyPI — a cosmetic `pip-tools` quirk, not
a sign the lock is wrong; trust the pinned versions below it, not the
header.)
...
pip install -e ".[dev]"      # latest compatible, matches README's quick start
pytest -q
```

To match CI's `lockfile` job exactly (e.g. to reproduce a CI-only
failure), install the pinned versions instead:
