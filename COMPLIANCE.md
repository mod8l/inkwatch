...
| Secure development lifecycle | CC8 | A.8.25–8.31 | Tests required, review checklist includes a security section (`CODE_REVIEW.md` §3), no secrets in source | No dependency vulnerability scanning (SCA — e.g. `pip-audit`/Dependabot), no SAST, no documented threat model. Cheapest of these gaps to close: turning on GitHub's free Dependabot alerts on this repo takes one settings toggle, no code change. |

## 3. If this becomes real: prioritized, concrete changes

In the order they'd start mattering, cheapest/most-code-relevant first:

1. **Dependency scanning** — `.github/dependabot.yml` is now in the repo (weekly version-update PRs for `pip` and the GitHub Actions used in CI). Security *alerts* still need someone with repo admin access to flip on under Settings → Security, same as branch protection in `AGENTS.md`.
