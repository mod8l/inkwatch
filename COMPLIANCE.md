...
| Supply-chain reproducibility | CC7.1, CC8 | A.8.25, A.8.32 | `requirements.txt`/`requirements-dev.txt` (pinned via `pip-tools`, see `AGENTS.md`) plus a CI check that fails if they drift from `pyproject.toml`, so exactly which dependency versions were verified together is reproducible and reviewable, not implicit in whatever `pip` resolves on a given day | No package/wheel hash pinning (`pip-compile --generate-hashes`) yet, which would additionally guard against a compromised/tampered artifact at the same pinned version. Not added now — worth it once this handles anything beyond the developer's own machine. |
