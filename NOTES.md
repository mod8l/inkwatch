...
- **Would change if:** Gad wants a keyboard answer channel before M5, or wants "which cell" questions reworded as a sequence of yes/no questions ("is it top left?") specifically so `y`/`n` can drive them.
...
#### M4.4 Two flagged divergences from a literal reading of §8's diagram
- `RESYNC --> WAIT_HUMAN` is the diagram's only drawn edge out of a successful resync. If the board was lost mid-`WAIT_AGENT_INK` (the agent's move already armed and announced), resuming into `WAIT_HUMAN` would silently drop that armed turn and wait on the wrong side — worse than what it's supposed to fix. `_resume_after_resync` instead resumes whichever phase matches whose turn it already was, re-arming the *same* target cell (no new `best_move` call) rather than picking a fresh one.
- The diagram doesn't draw `CALIBRATING --> BOARD_LOST` as reachable, and this build agrees by omission: there's no committed state to lose before the first board is even found, and `__main__.py`'s existing "board not found" banner already covers that wait. `CALIBRATING` stays immune to `BOARD_LOST`.
- **Flagging per CLAUDE.md ("stop and say so") rather than silently editing the diagram** — these read like the diagram being a simplified sketch rather than a considered "always go to WAIT_HUMAN" decision, but that's a call for Gad, not an unstated assumption. Happy to update `PRODUCT.md`'s mermaid diagram to match once confirmed, or to revert to the literal reading if the simplification was intentional.
...
| `ESCALATE` never actually calls a vision model | `escalation.py` is M5; every escalation-worthy read falls straight through to asking the human (§9's own specified no-model behavior, see M4.1) | Build `escalation.py` and plug a real call into `Session._resolve_escalate` |
| No keyboard `y`/`n` answer channel | Deferred — every M4 `ASK_HUMAN` case resolves through the page instead (M4.2); "which of two cells" doesn't map onto yes/no cleanly | A real answer channel, or reword multi-choice questions as a yes/no sequence |
