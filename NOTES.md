...
#### Process: bandit went red again once M2-M5 added `assert` statements and the macOS `say` fallback; fixed with a mix of a project-wide skip and two scoped suppressions
- **Found:** `bandit -r inkwatch tools` was clean at the end of M1 (noted above), but stayed silently red on every push since M2 added the first `assert` in `perception.py`'s classification code — `B101: assert_used`, one finding per `assert` statement, eventually 10 across `session.py`/`escalation.py` by the end of M5. Also flagged, from M1's `output.py`: `B404`/`B607`/`B603` on the `subprocess`-based macOS `say` fallback, and `B110` on the bare `except: pass` around it. None of this had been caught before because CI's own required-checks gate wasn't actually enforced (see `AGENTS.md`'s branch-protection note) — the check ran and failed on every PR, but nothing blocked on it. First surfaced for real when PR #6 (M4+M5) went up and GitHub reported it as a genuinely failing required check.
- **B101 (`assert`): skipped project-wide** (`bandit --skip B101` in `.github/workflows/tests.yml`), not suppressed line-by-line. These asserts document internal invariants the calling code's own control flow already guarantees (e.g. "`observation.cell_marks` is not `None` here because the caller already checked `observation.stable`") — never validation of untrusted input or a security boundary, which is the actual risk B101 exists to catch (asserts silently vanish under `python -O`). Bandit has no per-project "these aren't security-relevant" concept short of skipping the check, and 10 scattered `# nosec` comments would be noisier than the asserts themselves and easy to forget on the 11th one.
- **B110 (bare `except: pass`): a real fix, not a suppression.** `default_speak_fn`'s macOS-fallback failure now prints to stderr instead of silently vanishing — still never raises (TTS failing must never take down the frame loop, per `CLAUDE.md`), but a broken fallback is now visible when debugging instead of invisible. This resolved the finding on its own merits, no `# nosec` needed.
- **B404/B607/B603 (the `say` subprocess call itself): scoped `# nosec`.** List-form `argv` (no `shell=True`), a fixed, hardcoded command name, no untrusted input ever reaches it — a real false positive for this specific, narrow call site, not a pattern to bless everywhere `subprocess` might appear later. Learned the hard way that a `# nosec` comment with trailing prose on the same line gets bandit's parser trying to read every following word as a test ID (a wall of "not a test name or id" warnings, and the intended IDs not reliably matching) — the working form is a bare code (`# nosec` or `# nosec B404`) with any explanation moved to the comment line *above* it.
- **Would change if:** a future `assert` in this codebase ever *did* need to double as a security check (it shouldn't, per the hard rule that perception/session only ever escalate to asking a human on doubt, never silently trust unvalidated input) — then that one call site would want an explicit `if not(...): raise` instead of relying on an assert bandit's already told the whole project to ignore.
...
#### M6.1 `metrics.py` scores a *live* game's `events.jsonl`, not a `replay.jsonl`, against a hand-written label
- **The constraint:** every §13.1 metric except escalation rate and human-question rate needs ground truth — what was really drawn on the page and when — which no amount of re-running perception on the same recording can produce. `replay.py` (M5) already proves a change reproduces the same *decisions*; scoring the pipeline against its own re-derived output would prove nothing new about correctness.
- **Chosen:** `score_game(events, label)` reads the real game's `events.jsonl` (commits, questions, escalations, the final result — exactly what `__main__.py`'s `_log_tick`/`logger.log` already wrote while the game was actually played) and a `GameLabel` a human fills in by hand from watching the recording. `eval/README.md` has the schema and the step-by-step (record with `--record`, label, run `python -m inkwatch.metrics`).
- **Why `metrics.py` lives in `inkwatch/`, not `tools/` or `eval/`:** it's the `MET` box `ARCHITECTURE.md` already drew (dashed, "designed not built") — building it for real this milestone is exactly what moves that box from dashed to solid, not a one-off debug script like `tools/calibrate.py`. It's also plain-stdlib-plus-`pyyaml` logic with no camera or model dependency, so it's fully unit-testable the same way `rules.py`/`decision.py` are (`tests/test_metrics.py`, all synthetic).
- **Ground-truth timestamps use the recording's own `frame_ts` clock, not wall-clock video time.** `hand_left_frame_ts` in a label is read straight off the saved frame filename (`frames/commit_172731.842.png` → `172731.842`) rather than converted from a phone video's own timeline — `frame_ts` is `time.monotonic()` from that game's own process (arbitrary epoch, resets every run), so there's no shared clock to convert *from* without extra sync work a stopwatch-and-video setup doesn't give for free. This trades some precision (a labeler eyeballing "which frame is the hand actually gone" rather than a marked video timestamp) for exactness of the arithmetic once that frame is picked.
- **False triggers and the final board are self-reported in the label, not derived from the log.** Both *could* be computed by re-diffing frames against some independent baseline, but that would mean re-implementing a second, independent perception pass just to check the first one — more code, and not actually independent ground truth. Simpler and more honest: the labeler just writes down what they saw.
- **"Turn" (the denominator for escalation rate and human-question rate) is defined as one commit event.** `PRODUCT.md` §13.1 doesn't define it. A commit is the one thing every successfully-resolved turn produces regardless of path (straight through, via escalation, or via asking); a turn that's still open when the recording ends (an unresolved question or escalation with no following commit) isn't counted, since there both metrics apply. Flagged in Known limits — this slightly undercounts rate rather than overcounts it.
- **README.md's "Measured results" table was missing the "Human-question rate" row** that `PRODUCT.md` §13.1 defines — a doc/doc inconsistency, not a code divergence, but the kind of silent drift CLAUDE.md says to fix rather than carry forward. Added it in the same commit, plus made the "Cost per game" row explicit that it's an average token count (per D-M5.7), not a dollar figure.
- **Would change if:** the eval set ends up needing frame-level ink-ratio history rather than just the committed events (M5.5 already flagged this as the shape a future addition would take) — that's a separate, opt-in stream, not a change to this scorer's inputs.

#### M6.2 `test_e2e.py`: a full synthetic game through `run_frame`, calibration to `GAME_OVER` — still not what "end-to-end" usually means here
- **Asked directly:** whether the test suite had "very good e2e tests." It didn't — every existing test either checks one module in isolation or drives a couple of moves through synthetic frames (`test_replay.py`). Nothing played a *complete* game through the real perception → session → decision pipeline and checked it actually reaches `GAME_OVER` correctly.
- **Chosen:** `_play_full_game()` calls `run_frame` (the same function `__main__.py`'s live loop and `replay.py` both use) repeatedly over synthetic frames (`make_raw_frame`, from `test_replay.py`), scripting only the human's moves (a fixed cell-priority order) and reading the agent's actual reply cell back off `SessionResult.target_cell` to "draw" into the next frame — never hardcoding what the agent plays. Runs to `GAME_OVER` for both `agent_first=False` and `True`, and checks the terminal message doesn't flag a G5 desync.
- **This is not what CLAUDE.md means by needing a real camera.** Every mark is still painted by code onto a synthetic frame — no real hand, pen, lighting, or camera, and no real TTS/vision-model call. Said so explicitly, rather than calling this "e2e" and letting that word imply more than it covers; the module docstring says the same. What it adds over the existing tests is completeness (a whole game, not a move or two) and that the agent's replies are read back and re-injected rather than assumed, so it can't drift from whatever `decision.py` actually plays.
- **Found while building it:** `SessionResult.message` is only set on the tick something happened (M5.5's same "log-worthy tick" rule) — a settle loop that just reads the *last* of several repeated frames sees `message=None` even right after `GAME_OVER`, since the following quiet frames have nothing new to say. The test's `settle()` helper keeps the last tick that actually had a message, not just the final one.
- **Would change if:** a real camera dry run (still needed regardless — see Known limits) turns up a case this synthetic pipeline can't represent (e.g. partial occlusion mid-mark) — then that shape of frame would be worth adding here too, not just to `test_perception.py`'s narrower per-function tests.

#### M7.1 README dry run: the software-only half is measured and clean; the physical half still needs Gad
- **What "dry run" could actually mean from here:** `PRODUCT.md`'s M7 "done when" is "README dry run on a clean machine" — but a clean machine with a camera and a printer isn't something this sandbox has. Split the run into what's genuinely software (installable and measurable here) and what's physical (only Gad can do it), rather than skip the whole thing or fake a "10 minutes, confirmed" claim CLAUDE.md's own rule ("never write a measured number... unless it came from an actual run") would forbid.
- **What was actually run, in a fresh `.venv`, from a clean checkout, timed for real:** `python -m venv .venv` (3.8 s) → `pip install -e .` (13.2 s, `opencv-contrib-python` wheel already in pip's cache — a genuinely cold machine on a slow line will take longer to *download*, though the install step itself doesn't do more work) → `python tools/make_sheet.py` (0.2 s, writes `assets/board.png`) → `pytest -q` (4.4 s, 145 passed) → `cp .env.example .env` (instant, contents match what `README.md` §5 describes). None of these needed a camera, a printer, or sound hardware.
- **Also confirmed for real, not just by reading the code:** with no camera attached, `python -m inkwatch` and `tools/calibrate.py` both fail with the single clean line `Could not open camera '0'` and exit(1) — no crash, no traceback surfaced past OpenCV's own driver-probe warnings on stderr. With no `espeak-ng` installed (this sandbox has none), calling `output.py`'s `default_speak_fn` directly raises nothing — it falls through to the macOS-`say` fallback, which itself fails quietly and prints to stderr (per the M1 bandit fix, B110) — matching the README's own Linux/`espeak-ng` caveat and the "no crash" half of D8/M3.4's design, now actually exercised rather than just argued for.
- **Still not run, and can't be from here:** printing `assets/board.png` at true scale and checking it against a ruler; positioning a real camera; `tools/calibrate.py`'s none/ambiguous/marked check against 20 real manual marks; actually hearing speech; playing a full game on paper end to end; timing *that* against the "10 minutes" claim. These are exactly what README's Quick start and `PRODUCT.md` §13.2's acceptance checklist still need a human for.
...
**Update — third rung added (bare grid, no corner marks at all):** Gad's actual constraint turned out to be harder — no printer, no ruler, and no corner marks either; the board is a freehand 3×3 grid, with a soft shadow across it. Added `detect_grid_lines`: adaptive thresholding (ink judged against its *local* neighborhood, so a smooth shadow gradient neither fakes nor hides lines) → Hough segments → two dominant ~90°-apart directions → outermost *supported* line in each as borders → intersect for the four corners. A border must carry ≥10% of its family's total line length and ≥40% of the strongest cluster's support (an absolute-only threshold broke on Hough returning one segment vs two for the same drawn line — an implementation artifact, not a scene property). Hand-drawn corner estimates jitter a few px per frame, which reads as motion to the stability gate (P5), so both hand-drawn paths EMA-smooth corner points with a snap-through on real page moves. The reliability ladder is honest and documented: ArUco → corner squares → bare grid, each rung with strict acceptance so nothing hallucinates a board from desk clutter (all three pinned by synthetic tests, including a shadow-gradient frame).

**Update — hardened against the real desk frame:** first live test against Gad's actual drawing surfaced two failure modes synthetics didn't have. (1) Segments of *one* thick, slightly-tilted hand line have midpoints scattered over 35+ px, so midpoint-projection clustering shattered one physical border into several low-support clusters — clustering now runs on each segment's own fitted normal form (θ, ρ), where pieces of the same line always land together. (2) A strong shadow's hard edge is a long, well-supported line that out-votes a real border — candidates are now arbitrated on grid properties a shadow can't fake (the two families' spacings must be roughly square; a real grid's quad contains the grid's inner lines). (3) The corner-square path's global Otsu threshold picked the shadow itself as "dark," merging squares inside it into one giant shadow blob — it now uses the same local-contrast adaptive threshold as the other paths. Verified on the actual frame: four squares painted into the shadowed scene all decode and rectify cleanly. The bare-grid rung still declines Gad's current drawing — its outer border is curved freehand, and straight-line detection can't fit it; that's the documented limit of the no-marks path, not a silent failure.
...
| §13's metrics table is entirely `TODO` — no eval games recorded yet | This build has no camera; `metrics.py`/`eval/README.md` (M6) are the scoring tooling, not the data | Gad records ≥10 games per `eval/README.md`, labels them, runs `python -m inkwatch.metrics`, pastes the result into README.md |
| `metrics.py`'s "turn" (escalation-rate / human-question-rate denominator) counts only commit events | §13.1 doesn't define "turn"; a turn still open when the recording ends (no following commit) isn't counted (M6.1) | Confirm this reads as intended once real eval games exist; a turn count independent of whether it resolved would need session.py to expose one directly |
| `hand_left_frame_ts` ground truth is a labeler's eyeballed read of which saved frame shows the hand just gone | No independent, more precise "hand left the page" signal is logged (M6.1) | A dedicated occlusion-cleared event in `events.jsonl` if this turns out too noisy against real footage |
| Even `test_e2e.py`'s full-game test (M6.2) never sees a real hand, pen, camera, or spoken word | Every mark is painted onto a synthetic frame by code; no test suite can substitute for the real thing CLAUDE.md says can't be verified here | A real camera dry run per README's Quick start — still the one thing only Gad can do |
| README's "about 10 minutes" setup estimate is unconfirmed against a real stranger/clean machine (M7.1) | Only the software half (venv, install, `make_sheet.py`, `pytest`) was actually timed here — printing, camera positioning, and calibration are physical and untimed | Gad times his own first run start to finish and corrects the estimate if it's off |
